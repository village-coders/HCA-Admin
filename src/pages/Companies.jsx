...
const Companies = () => {
  ...
  const getTabCounts = () => {
    return {
      ...
      'no-applications': companies.filter(c => {
         const stats = getCompanyStats(c.registrationNo);
         return stats.totalApps === 0;
      }).length,
    };
  ...
  const tabs = [
    { id: 'applications', label: 'Company List', count: tabCounts.applications },
    { id: 'all', label: 'All Companies', count: tabCounts.all },
    { id: 'lists', label: 'Approved Products', count: tabCounts.lists },
    { id: 'non-approved', label: 'No Approved Products', count: tabCounts['non-approved'] },
    { id: 'no-applications', label: 'Signup', count: tabCounts['no-applications'] },
  ];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {companies.length} registered companies • {companies.filter(c => getCompanyStats(c.registrationNo).approvedProds > 0).length} approved products
            </p>
          </div>
