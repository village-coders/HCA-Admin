            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 text-black">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full">
                  <div className="flex-1 bg-gray-50 p-6 rounded-2xl border border-gray-100 flex flex-col items-center justify-center text-center">
                  ...
                  {selectedLogsheet.labResult && (
                    <div className="flex-1 bg-gray-50 p-6 rounded-2xl border border-gray-100 flex flex-col items-center justify-center text-center">
                      <FileText className="w-10 h-10 text-gray-300 mb-2" />
                      <p className="text-sm font-semibold text-gray-600 mb-4">Lab Result Document</p>
                      <a 
                        href={resolveUrl(selectedLogsheet.labResult)} 
                        target="_blank" 
                        rel="noreferrer"
                        className="px-6 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-[#00853b] hover:bg-gray-50 shadow-sm transition-all"
                      >
              ...
              {selectedLogsheet.additionalDocuments && selectedLogsheet.additionalDocuments.length > 0 && (
                <div className="space-y-4">
                  <h3 className="font-bold text-gray-900 border-b pb-2">Additional Documents</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {selectedLogsheet.additionalDocuments.map((doc, idx) => {
                      const parts = doc.split('/');
                      const filename = parts[parts.length - 1] || `Document ${idx + 1}`;
                      const cleanName = filename.replace(/^addoc-[a-f0-9]+-\d+-/, '');
                      return (
                        <div key={idx} className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col items-center justify-center text-center">
                          <FileText className="w-8 h-8 text-gray-300 mb-2" />
                          <p className="text-xs font-semibold text-gray-600 mb-3 truncate w-full max-w-[150px]" title={cleanName}>
                            {cleanName}
                          </p>
                          <a 
                            href={resolveUrl(doc)} 
                            target="_blank" 
                            rel="noreferrer"
                            className="px-4 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-bold text-[#00853b] hover:bg-gray-50 shadow-sm transition-all"
                          >
                            View Document
                          </a>
                        </div>
