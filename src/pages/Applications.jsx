...
const Applications = () => {
  ...
  const filteredApplications = applications.filter(app => {
    ...
    if (filter.company && filter.company.trim()) {
      ...
      const matchCompanyObjName = typeof app.company === 'object'
        ? (app.company?.companyName?.toLowerCase().includes(searchTerm) || app.company?.fullName?.toLowerCase().includes(searchTerm))
        : false;

      const rawCompId = typeof app.company === 'string' ? app.company : (app.companyId || app.company?._id || app.company?.id);
  ...
  const handleViewDetails = async (appId) => {
    ...
    setSelectedAppProducts([]);
    try {
      const token = JSON.parse(localStorage.getItem('accessToken'));
      const [application, prodRes] = await Promise.all([
        getApplicationById(appId),
        fetch(`${import.meta.env.VITE_BASE_URL}/products/admin-all?applicationId=${appId}`, {
          headers: { Authorization: `Bearer ${token}` }
        }).then(r => r.json()).catch(() => ({ products: [] }))
      ]);
      if (application) {
        setSelectedApplication(application);
        setSelectedAppProducts(prodRes.products || []);
  ...
  const ViewApplicationModal = () => {
    ...
    const detailTabs = [
      ...
      { id: 'products', label: `Products${selectedAppProducts.length > 0 ? ` (${selectedAppProducts.length})` : ''}`, icon: Package },
    ];
      ...
      <div className="mb-6">
        <div className="flex flex-wrap gap-2">
          {tabs.map((tab) => (
            ...
            >
              {tab.label}
              <span className={`ml-2 px-1.5 py-0.5 text-xs rounded-full ${activeTab === tab.id ? 'bg-white/20' : 'bg-gray-100'
                }`}>
                {tab.count}
              </span>
            </button>
              ...
              <tbody className="divide-y divide-gray-200">
                {paginatedApplications.length > 0 ? (
                  paginatedApplications.map((app) => {
                    ...
                    const StatusIcon = statusConfig.icon;
                    // Keep typeConfig just in case, but we will render category
                    ...
                    return (
                      <tr key={appId} className="hover:bg-gray-50">
                        ...
                        <td className="p-4">
                          <span className="inline-flex px-3 py-1 text-xs font-medium rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200">
                            {app.category || 'N/A'}
                          </span>
                        </td>
                        <td className="p-4">
