...
const Invoices = () => {
  ...
  const filteredInvoices = invoices.filter(invoice => {
    const matchesSearch = 
      ...
      invoice.applicationId?.applicationNumber?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleIssueSubmit = async (e) => {
    ...
    const result = await issueInvoice(selectedInvoice._id, parseFloat(issueAmount));
    ...
    <div className="p-4 lg:p-8 pt-20 lg:pt-8 min-h-screen bg-gray-50/50">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage and track all application payments</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input 
              type="text" 
              placeholder="Search by invoice #, company, or application..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">{invoices.filter(i => i.status === 'Requested').length}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4">
                  ...
                  <tr key={invoice._id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm font-medium text-blue-600 bg-blue-50 px-2.5 py-1 rounded-md">
                        {invoice.applicationId?.applicationNumber || 'N/A'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
      ...
      {isIssueModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm" onClick={() => setIsIssueModalOpen(false)}></div>
            ...
            <form onSubmit={handleIssueSubmit} className="p-6">
              <div className="mb-6 bg-blue-50 p-4 rounded-xl border border-blue-100">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-blue-900">{selectedInvoice?.userId?.companyName}</h4>
                    <p className="text-xs text-blue-700 mt-1">Application: {selectedInvoice?.applicationId?.applicationNumber}</p>
                    <p className="text-xs text-blue-700">Request: {selectedInvoice?.invoiceNumber}</p>
                  </div>
                </div>
              </div>
